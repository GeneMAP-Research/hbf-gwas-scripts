...
if (length(args) < 4) {
   ...
   quit(save="no")
     ...
     svg_plt_name <- paste0(result_a_name, "_", result_b_name, "_manhattan.svg")
     ...
     plot_mirror_manhattan <- function(x=data_a, y=data_b) {
          ...
          svg(svg_plt_name, height = 5, width=8)
          par(fig=c(0,1,0.41,1))
          print(paste0("Plotting manhattan for ", result_a_name, "..."), quote=F)
          manhattan(data_a, ylim = c(0, maxylim_a), col = c("gray60", "dodgerblue4"), genomewideline = F, suggestiveline = F, cex.axis = 0.6, logp=T, xlab=NA, bg="white")
